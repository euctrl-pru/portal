avg_delay_grouped <- function (year_input, ref_year){
  ...
  all_details <- all_details %>%
    ...
    mutate(entity_group = case_when(
      ...
      entity_name %in% c("Belgocontrol","DSNA","DFS","LVNL","MUAC","Skyguide")    ~ "FABEC",
  ...
  all_summaries_yyyy <- all_summaries %>%
    filter(yyyy == year_input) %>%
    filter(!(entity_name == "ANA LUX") & !(entity_name == "SES Area (RP1)")) %>%
  ...
  all <- inner_join(all_yyyy, ref, by = c("entity_name","entity_group"))
  
  # Create plot -------------------------------------------------------------------------  
  g <- ggplot(all, aes(x = entity_name, y = delay/flt_tot, fill = delay_group))
