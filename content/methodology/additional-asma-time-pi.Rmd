...
* SES: IR691/2010 [@eu:691/2010] and IR390/2013 [@eu:390/2013], and
* EUROCONTROL: performance review reporting [@atmap-2009].


This web page is generated from [@pru-adasma-pi].
...
Airport Operator Data Flow [@ir691-apt] standard for
...
The following list summarises the status of the indicator:
...
* Used in
    ...
    - SES eDashboard [ RP1 [@prb:dashboardRP1] and RP2 [@prb:dashboardRP2] ]
...
places at which queuing takes place, as illustrated in the Figure \@ref(fig:adasma-01):
...
(ref:adasma-01) Outbound and Inbound Traffic Queues from a Flight Phase Perspective.


```{r adasma-01, out.width=c('500px', '300px'), fig.cap ='(ref:adasma-01)'}
knitr::include_graphics("/images/adasma-01.png")
```
...
reference period sample. This approach is depicted in Figure \@ref(fig:adasma-02).
Based on regular
reporting, metrics are derived for the respective reporting month. The current
measurements are compared to a nominal reference to address the level of
efficiency. The reference is derived from the statistical analysis of a
...
(ref:adasma-02) Performance Measurement Approach.


```{r adasma-02, out.width=c('650px', '650px'), fig.cap='(ref:adasma-02)'}
knitr::include_graphics("/images/adasma-02.png")
```


The indicator is defined, see Equation \@ref(eq:AcASMA), as the difference between the
ASMA transit time (actual
ASMA time) and the unimpeded ASMA time, based on ASMA transit times in periods
of low traffic.

$$
\begin{equation} 
textrm{Actual ASMA Time} = textrm{Unimpeded ASMA Time} + textrm{Additional ASMA Time}
(\#eq:AcASMA)
\end{equation} 
$$
...
(ref:adasma-03) Conceptual approach for Additional ASMA Time.


```{r adasma-03, out.width=c('450px', '450px'), fig.cap = '(ref:adasma-03)'}
knitr::include_graphics("/images/adasma-03.png")
```
...
visualization of arrival radar tracks (see Figure \@ref(fig:adasma-sectors))
and the aforementioned entry points.


(ref:adasma-sectors) ASMA sectors and bearing.


```{r adasma-sectors, out.width=c('450px', '450px'), fig.cap = '(ref:adasma-sectors)'}
...
(ref:adasma-05) Logical steps of Additional ASMA Time calculation.


```{r adasma-05, out.width=c('900px', '900px'), fig.cap = '(ref:adasma-05)'}
knitr::include_graphics("/images/adasma-05.png")
```
...
(ref:adasma-06) Logical steps of unimpeded ASMA Time calculation.


```{r adasma-06, out.width=c('1000px', '1000px'), fig.cap = '(ref:adasma-06)'}
knitr::include_graphics("/images/adasma-06.png")
```
...
(ref:adasma-stepA) Step A.


```{r adasma-stepA, out.width=c('750px', '750px'), fig.cap = '(ref:adasma-stepA)'}
knitr::include_graphics("/images/adasma-stepA.png")
```
...
* flights with no actual ASMA time or an actual ASMA time of more than 2 hours
  are excluded, i.e. flights with $\textrm{AcASMA} < 120 \textrm{min}$ are taken into
  account;
...
(ref:adasma-stepB) Step B.


```{r adasma-stepB, out.width=c('700px', '700px'), fig.cap = '(ref:adasma-stepB)'}
knitr::include_graphics("/images/adasma-stepB.png")
```


$\textrm{UASMA}(c_i)$, the **unimpeded ASMA time**, is a calculated constant for each
...
(ref:adasma-stepC) Step C.


```{r adasma-stepC, out.width=c('750px', '750px'), fig.cap = '(ref:adasma-stepC)'}
knitr::include_graphics("/images/adasma-stepC.png")
```
...
* $\textrm{ET}(f_{c_i})$ ASMA entry time. The time of last entry of the flight
  $f_{c_i}$ in its ASMA sector [time],
* $\textrm{ALDT}(f_{c_i})$ Actual Landing Time of flight $f_{c_i}$ [time],

The $\textrm{AcASMA}(f_{c_i})$, the actual ASMA transit time for a flight
$f_{c_i}$, is defined as the elapsed time between the time of the __last__ entry
of the flight $f_{c_i}$ in its ASMA sector, $\textrm{ET}(f_{c_i})$ [time], and its
Actual Landing Time $\textrm{ALDT}(f_{c_i})$ [time].

$$
\textrm{AcASMA}(f_{c_i}) = \textrm{ALDT}(f_{c_i}) - \textrm{ET}(f_{c_i}), [min]
$$


Then, the **Additional ASMA Time per flight** $\textrm{AdASMA}(f_{c_i})$ is
calculated for each flight $f_{c_i}$ as the difference between the Actual ASMA
Transit Time $\textrm{AcASMA}(f_{c_i})$ of the flight and the Unimpeded ASMA Time
$\textrm{UASMA}(c_i)$ for the Grouping $c_i$ to which the flight belongs.

$$
\textrm{AdASMA}(f_{c_i}) = \textrm{AcASMA}(f_{c_i}) - \textrm{UASMA}(c_i), [min]
$$


### Step D: Calculation of the Additional ASMA Time per airport

(ref:adasma-stepD) Step D.


```{r adasma-stepD, out.width=c('600px', '600px'), fig.cap = '(ref:adasma-stepD)'}
knitr::include_graphics("/images/adasma-stepD.png")
```
...
The **additional ASMA time for a given airport** $\textrm{AdASMA}_j$ is the average
of the additional ASMA time $\textrm{AdASMA}(f_j)$, for all the flights $f_j$ at
that airport that have an unimpeded reference, in the sample $N_j$.

$$
\textrm{AdASMA}_j = \frac{ textrm{AdASMA}(f_j)}{N_j}, [\textrm{min}/\textrm{IFR arrival flight}].
$$
...
(ref:adasma-step1) Step 1.


```{r adasma-step1, out.width=c('700px', '700px'), fig.cap = '(ref:adasma-step1)'}
knitr::include_graphics("/images/adasma-step1.png")
```


A filter is applied so that only flights with $\textrm{AcASMA}(f_{c_i}) < 120\, \text{min}$
...
(ref:adasma-step2a) Step 2a.


```{r adasma-step2a, out.width=c('400px', '400px'), fig.cap = '(ref:adasma-step2a)'}
knitr::include_graphics("/images/adasma-step2a.png")
```


Then, $\textrm{AcASMA}(f_{c_i})$, the Actual ASMA transit time for a flight
...
(ref:adasma-step2b) Step 2b.


```{r adasma-step2b, out.width=c('700px', '700px'), fig.cap = '(ref:adasma-step2b)'}
knitr::include_graphics("/images/adasma-step2b.png")
```




For each flight $f_{c_i}$, a **congestion level** $\textrm{seq}(f_{c_i})$ can be
determined [count]. The congestion level is the number of other landings during
the time interval:
[ASMA entry time $\textrm{ET}(f_{c_i})$, actual landing time $\textrm{ALDT}(f_{c_i})$] and
then subtracting the flight itself. Thus, for all flights $f$ landing in that
time interval:

$$
\text{seq}(f_{c_i}) = \sum f - 1 \quad \forall f \textrm{ such that }
...
(ref:adasma-step3) Step 3.


```{r adasma-step3, out.width=c('700px', '700px'), fig.cap = '(ref:adasma-step3)'}
knitr::include_graphics("/images/adasma-step3.png")
```


#### Step 3a: determination of airport congestion index

At airport level, a constant known as the congestion limit $\textrm{cl}$ is chosen as:
...
(ref:adasma-step3b) Step 3b.


```{r adasma-step3b, out.width=c('450px', '450px'), fig.cap = '(ref:adasma-step3b)'}
knitr::include_graphics("/images/adasma-step3b.png")
```
...
(ref:adasma-step3d) Step 3d.


```{r adasma-step3d, out.width=c('350px', '350px'), fig.cap = '(ref:adasma-step3d)'}
knitr::include_graphics("/images/adasma-step3d.png")
```
...
$\textrm{cl}$, a constant defined in Step 3a.
...
(ref:adasma-step4) Step 4.


```{r adasma-step4, out.width=c('650px', '650px'), fig.cap = '(ref:adasma-step4)'}
knitr::include_graphics("/images/adasma-step4.png")
```
...
(ref:adasma-step5) Step 5.


```{r adasma-step5, out.width=c('700px', '700px'), fig.cap = '(ref:adasma-step5)'}
knitr::include_graphics("/images/adasma-step5.png")
```


The unimpeded ASMA time $\textrm{UASMA}(c_i)$ for a grouping $c_i$ is a
...
statistical cluster analysis of the distribution of the inbound traffic.
Figure \@ref(fig:asmasectors) shows the sectorization for Heathrow.


(ref:asmasectors) ASMA sectors and bearing.


```{r asmasectors, out.width=c('350px', '350px'), fig.cap = '(ref:asmasectors)'}
knitr::include_graphics("/images/asmasectors.jpg")
```
...
(ref:adasma-08) Airport Data Flow process scheme including activities and actors involved.


```{r adasma-08, out.width=c('800px', '800px'), fig.cap = '(ref:adasma-08)'}
knitr::include_graphics("/images/adasma-08.png")
```
...
(ref:adasma-09) Quality Performance Areas and corresponding Quality Indicators.

```{r adasma-09, out.width=c('700px', '700px'), fig.cap = '(ref:adasma-09)'}
knitr::include_graphics("/images/adasma-09.png")
```
