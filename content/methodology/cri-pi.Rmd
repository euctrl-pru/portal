...
CRI represents a cumulative risk value calculated aggregating all reported, assessed and severity classified safety-related incidents to form an index. This measure of risk exposure is based on probability and severity that considers the human perception of equivalent risk.
...
the human perception of their risk it is possible to calculate
CRI for each Entity separately.
The simple formula to calculate CRI is presented below. 
...
In addition, each type of weight selection has predefined weight ranges, to allow for incremental Severity classification order based on human perception of risk (from accident to Severity C incident, i.e. from high risk to low risk). In other words, each range has an expectation value associated with it. See details in the full methodology report [@performancereviewcommissionCompositeRiskIndex2019]
...
## Further Information
For further information please refer also to detailed report explaining development methodology [@performancereviewcommissionCompositeRiskIndex2019].

## Working example
Script calculating CRI using explained methodology and sample data are available for testing purposes [here]
[crizip].

[crizip]: /static/download/other/CRI-External.zip
